<div>
    @if ($ticket->status == 'open')
        <div class="bg-white">
            <h2 class="text-lg font-semibold font-display text-gray-700 tracking-tight mx-3 py-1.5">Close ticket
            </h2>
            ...
            <div class="flex items-start px-5 py-5">
                ...
                <div class="text-center ml-4 sm:text-left">
                    <p class="text-gray-500">This will action will close <span class="font-medium">{{ $ticket->title }}
                        </span>
    ...
    @else
        <div class="bg-white">
            <h2 class="text-lg font-semibold font-display text-gray-700 tracking-tight mx-3 py-1.5">Open ticket
            </h2>
            <div class="border-t border-gray-300 mt-1"></div>
            <div class="flex items-start px-5 py-5">
                <div class="mx-auto flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-red-100">
                ...
                <div class="text-center ml-4 sm:text-left">
                    <p class="text-gray-500">This will action will reopen <span class="font-medium">{{ $ticket->title }}
                        </span>
                        by <span class="font-medium"> {{ $ticket->user->name }}</span>.</p>
                </div>
            </div>
            ...
            <div class="bg-slate-100">
                <div class="px-4 py-4">
                    <x-button sm wire:click="$dispatch('closeModal')" flat label="Cancel" />
                    <x-button sm wire:click="openTicket" class="ml-2 font-medium leading-6" green label="Close" />
                </div>
            </div>
        </div>
    @endif
